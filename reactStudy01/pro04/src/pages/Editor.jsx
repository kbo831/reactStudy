...
4) getFormattedData() utils에 인자로 주어 반환되는 형식의 날짜 데이터를 얻을 수 있음*/
    ...
    //이벤트 핸들러를 통해 초기값 재설정 시, 컴포넌트가 렌더링될 때마다, 함수도 재생성 되기때문에 useCallback()을 통해서
    // 마운트 시에만 생성되게 지정할 수 있지만, state의 상태값도 변하지 않게됨 => 초기값 갱신 반환 함수를 함수형업데이트 방식으로 작성해주어  초기값 변경하기
    const handlerChangeEmotion = useCallback((emotionId) =>{
        setState((state)=>({
            ...state,
            emotionId
        }));
